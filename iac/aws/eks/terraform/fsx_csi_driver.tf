...
resource "aws_iam_policy" "fsx_csi_driver_controller_policy" {
  name        = var.fsx_csi_controller_role_policy_name_prefix
  description = format("Allow CSI Driver to manage AWS FSX resources")
  path        = "/"
  policy      = file("fsx_csi_driver_policy.json")
  tags        = merge(var.fsx_csi_tags, var.tags)
}
...
module "fsx_controller_role" {
  ...
  role_name_prefix              = var.fsx_csi_controller_role_name
  provider_url                  = module.eks.cluster_oidc_issuer_url
  role_policy_arns              = [aws_iam_policy.fsx_csi_driver_controller_policy.arn]
